...
; Source: http://www.loremipsum.de/downloads/version7.txt
LoremText =
(
...
MaxLength := StrLen(LoremText)
Gui, Add, Slider, ToolTip NoTicks Range5-%MaxLength% AltSubmit vSlidePos, 120
...
TempData := SubStr(LoremText, 1, SlidePos)
Clipboard := TempData

MaxLen := 77
ToDisplay := TempData
DisplayLen := StrLen(ToDisplay)
If (DisplayLen > MaxLen + 3)
	ToDisplay := SubStr(ToDisplay, 1, MaxLen//2) 
	. "..." 
	. SubStr(ToDisplay, MaxLen // 2 * -1  - 1)
TrayTip, Copied, %ToDisplay%
ExitApp
return
...
GuiControlGet, SlidePos
Clipboard := SubStr(LoremText, 1, SlidePos)
